...
import {
  Package, Plus, Pencil, Trash2, Loader2, Search, Filter,
  TrendingUp, TrendingDown, AlertTriangle, FileDown,
  BarChart3, Grid, List, Star, Archive, Eye, Copy,
  RefreshCw, DollarSign, Box, Activity
} from 'lucide-react';
...
const Products = () => {
  ...
  const stats = useMemo(() => {
    ...
    const outOfStock = safeProducts.filter(p => (p.quantity || 0) === 0).length;
    const avgPrice = totalProducts > 0 ? totalValue / safeProducts.reduce((sum, p) => sum + (p.quantity || 0), 0) : 0;

    return { totalProducts, totalValue, lowStock, outOfStock, avgPrice };
  }, [products]);
        ...
        <aside className="w-64 bg-white border-l border-slate-200 min-h-screen p-6 hidden lg:block">
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium text-slate-600">إجمالي المنتجات</span>
              ...
              <div className="bg-green-50 p-4 rounded-xl border border-green-100">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium text-slate-600">القيمة الإجمالية</span>
              ...
              <div className="bg-orange-50 p-4 rounded-xl border border-orange-100">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium text-slate-600">نواقص</span>
            ...
            <div className="pt-6 border-t">
              <h3 className="text-sm font-semibold mb-3 text-slate-800">التصنيفات</h3>
              <div className="space-y-1">
                {categories.map(cat => (
                  <button
                    key={cat}
                    onClick={() => setFilterCategory(cat)}
                    className={`w-full text-right px-3 py-2 rounded-lg transition-all text-sm flex items-center justify-between ${filterCategory === cat
                  ...
                  >
                    ...
                    {cat !== 'all' && (
                      <span className="text-xs bg-white px-2 py-0.5 rounded-full border border-slate-200">
            ...
            {/* Quick Actions */}
            <div className="pt-6 border-t">
              <h3 className="text-sm font-semibold mb-3 text-slate-800">إجراءات سريعة</h3>
                    ...
                    <TableBody>
                      {filteredProducts.length === 0 ? (
                        ...
                        filteredProducts.map((product) => {
                          ...
                          const profit = ((product.price || 0) - (product.cost || 0)) * (product.quantity || 0);
                            ...
                            <TableRow key={product.id} className="hover:bg-slate-50 transition-colors">
                              ...
                              <TableCell>
                                <div>
                                  ...
                                  {product.cost && (
                                    <p className="text-xs text-slate-400">تكلفة: {product.cost}</p>
                                  )}
                                </div>
                              </TableCell>
                              <TableCell>
